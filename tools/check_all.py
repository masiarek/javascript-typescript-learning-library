...
GATES: list[tuple[str, list[str]]] = [
    # Runs tsc over every TypeScript example first, then every example.
    ("examples", [sys.executable, "tools/run_examples.py", "--check"]),
    ("link style", [sys.executable, "tools/check_link_style.py"]),
    ("pages and indexes", [sys.executable, "tools/check_pages.py"]),
...
def borrow_node_modules(tmp: pathlib.Path) -> None:
    """Give an extracted tree the checkout's node_modules (the pinned tsc).

    node_modules is never committed, so `git archive` leaves it out, and the
    examples gate would stop at "run npm ci". A symlink is enough: the runner
    reaches tsc and @types through it, and mkdocs excludes node_modules/.
    """
    if (REPO / "node_modules").is_dir() and not (tmp / "node_modules").exists():
        (tmp / "node_modules").symlink_to(REPO / "node_modules", target_is_directory=True)
...
def committed_tree() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        borrow_node_modules(pathlib.Path(tmp))
        head = subprocess.run(
