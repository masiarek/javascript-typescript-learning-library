...
2026-09-07 -- the published `11_Tools` page offered *awk* as the page after it
while the sidebar beside it read *grep* first. Nothing caught it because each
half is internally consistent, `--strict` has no opinion about a hook that
reorders a nav without re-linking it, and only a reader who already knew what
came next could tell. This library inherited the gate with the hook.
...
def stale_entries() -> list[tuple[str, str | None, str]]:
    ...
    skip = {".git", "site", ".venv", "node_modules", ".claude"}
    folders = {d.name for d in REPO.rglob("*")
               if d.is_dir() and not skip.intersection(d.relative_to(REPO).parts)}
