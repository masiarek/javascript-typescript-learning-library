...
**Node 24 or later, nothing from npm.** An example uses the language and Node's built-in modules (`node:fs`, `node:util`, `node:test`...), never a package — the only package in the repo is the TypeScript compiler, pinned in `package.json`. A lesson about a library (a validator, a date library, a bundler) names it, links it, and builds the smallest standard version of what it does.
...
Four things repeat what the pages say, and `tools/check_pages.py --fix` regenerates all of them: each chapter's table of pages (between `<!-- lessons -->` markers), the front page's chapter tables and progress line, and [KEYWORDS.md](KEYWORDS.md). CI fails when any of them is stale, when a lesson folder is missing from `NAV_ORDER`, when a stub has an output block, and when a page is neither a stub nor a checked lesson.
