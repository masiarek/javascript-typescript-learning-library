#!/usr/bin/env bash
# no-implicit-coercion: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_implicit_coercion_bad_js.js values_lints_no_implicit_coercion_good_js.js; do
  echo "\$ node $program"
  node "$program"
