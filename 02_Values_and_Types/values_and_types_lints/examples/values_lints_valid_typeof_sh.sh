#!/usr/bin/env bash
# valid-typeof: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_valid_typeof_bad_js.js values_lints_valid_typeof_good_js.js; do
  echo "\$ node $program"
  node "$program"
