#!/usr/bin/env bash
# no-constant-binary-expression: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_constant_binary_expression_bad_js.js values_lints_no_constant_binary_expression_good_js.js; do
  echo "\$ node $program"
  node "$program"
