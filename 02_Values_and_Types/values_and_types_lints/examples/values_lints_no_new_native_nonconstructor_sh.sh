#!/usr/bin/env bash
# no-new-native-nonconstructor: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_new_native_nonconstructor_bad_js.js values_lints_no_new_native_nonconstructor_good_js.js; do
  echo "\$ node $program"
  node "$program"
