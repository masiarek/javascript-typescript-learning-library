#!/usr/bin/env bash
# no-compare-neg-zero: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_compare_neg_zero_bad_js.js values_lints_no_compare_neg_zero_good_js.js; do
  echo "\$ node $program"
  node "$program"
