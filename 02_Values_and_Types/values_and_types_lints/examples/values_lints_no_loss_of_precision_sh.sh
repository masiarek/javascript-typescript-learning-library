#!/usr/bin/env bash
# no-loss-of-precision: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_loss_of_precision_bad_js.js values_lints_no_loss_of_precision_good_js.js; do
  echo "\$ node $program"
  node "$program"
