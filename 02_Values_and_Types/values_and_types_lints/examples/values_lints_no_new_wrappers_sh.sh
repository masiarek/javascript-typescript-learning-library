#!/usr/bin/env bash
# no-new-wrappers: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_new_wrappers_bad_js.js values_lints_no_new_wrappers_good_js.js; do
  echo "\$ node $program"
  node "$program"
