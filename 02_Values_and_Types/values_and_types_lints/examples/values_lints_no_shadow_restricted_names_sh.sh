#!/usr/bin/env bash
# no-shadow-restricted-names: run the bad program and the good one, and lint each with only this
...
here=$(pwd -P)
for program in values_lints_no_shadow_restricted_names_bad_js.js values_lints_no_shadow_restricted_names_good_js.js; do
  echo "\$ node $program"
  node "$program"
